...
if (!empty($data)) {
    ...
    foreach ($data as $ride) {
        ...
        $time = $ride['formatted_time']; // Keep the same time
        ...
        $sql = "INSERT INTO ride (date, day, time, pick_up_point, drop_off_point, slots_available, price, driver_id, vehicle_id) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";

        $stmt = mysqli_prepare($conn, $sql);
        mysqli_stmt_bind_param($stmt, "sssssidii", $newDate, $day, $time, $pickup, $dropoff, $slots, $price, $driverID, $vehicleID);
